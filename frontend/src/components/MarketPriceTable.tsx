        ...
        <table className="w-full text-left text-xs border-collapse">
          <thead>
            <tr className="bg-slate-50/80 border-b border-slate-200 text-slate-600 font-extrabold uppercase tracking-wider">
              <th className="py-3 px-4 cursor-pointer hover:bg-slate-100" onClick={() => onSort('commodity')}>
                <div className="flex items-center gap-1.5">
                  <span>Commodity</span>
                  {getSortIcon('commodity')}
                </div>
              </th>
              <th className="py-3 px-4 cursor-pointer hover:bg-slate-100" onClick={() => onSort('state')}>
                <div className="flex items-center gap-1.5">
                  <span>State</span>
                  {getSortIcon('state')}
                </div>
              </th>
              <th className="py-3 px-4 cursor-pointer hover:bg-slate-100" onClick={() => onSort('district')}>
                <div className="flex items-center gap-1.5">
                  <span>District</span>
                  {getSortIcon('district')}
                </div>
              </th>
              <th className="py-3 px-4 cursor-pointer hover:bg-slate-100" onClick={() => onSort('market')}>
                <div className="flex items-center gap-1.5">
                  <span>Market (APMC)</span>
                  {getSortIcon('market')}
                </div>
              </th>
              <th className="py-3 px-4 text-right cursor-pointer hover:bg-slate-100" onClick={() => onSort('min_price')}>
                <div className="flex items-center justify-end gap-1.5">
                  <span>Min Price</span>
                  {getSortIcon('min_price')}
                </div>
              </th>
              <th className="py-3 px-4 text-right cursor-pointer hover:bg-slate-100" onClick={() => onSort('max_price')}>
                <div className="flex items-center justify-end gap-1.5">
                  <span>Max Price</span>
                  {getSortIcon('max_price')}
                </div>
              </th>
              <th className="py-3 px-4 text-right cursor-pointer hover:bg-slate-100" onClick={() => onSort('modal_price')}>
                <div className="flex items-center justify-end gap-1.5">
                  <span>Modal Price</span>
                  {getSortIcon('modal_price')}
                </div>
              </th>
              <th className="py-3 px-4 text-center cursor-pointer hover:bg-slate-100" onClick={() => onSort('arrival_date')}>
          ...
          <tbody className="divide-y divide-slate-100">
            {records.map((r, idx) => (
              <tr key={idx} className="hover:bg-emerald-50/30 transition-colors">
                ...
                <td className="py-3.5 px-4 text-slate-700 font-semibold">{r.state}</td>
                <td className="py-3.5 px-4 text-slate-600 font-medium">{r.district}</td>
                <td className="py-3.5 px-4 text-slate-900 font-bold">{r.market}</td>
